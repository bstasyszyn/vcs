...
func (s *revocationList2021Processor) CreateVCStatus(index, vcID, statusPurpose string, additionalFields ...vcapi.Field) *verifiable.TypedID {
...
func (s *revocationList2021Processor) CreateVC(vcID string, listSize int, statusPurpose string, profile *vcapi.Signer) (*verifiable.Credential, error) {
	if statusPurpose != StatusPurposeRevocation {
		return nil, fmt.Errorf("unsupported status purpose: %s", statusPurpose)
	}
...
func (s *revocationList2021Processor) GetStatusPurpose(vcStatus *verifiable.TypedID) (string, error) {
